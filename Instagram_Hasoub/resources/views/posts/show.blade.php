            ...
            <div class="col-start-2 col-span-3 border border-solid border-gray-300">
                <div class="grid grid-cols-5">
                    <div class="col-span-3">
                        <div class="flex justify-center"><img src="/storage/{{$post->image_path}}" id="postImage" alt="" style="max-height:80vh"></div>
                    </div>
                    <div class="col-span-2 bg-white flex flex-col">
                        <div class="flex flex-row p-3 border-b border-solid border-gray-300 items-center justify-between" id="sec1">
                            <div class="flex flex-row items-center">
                                ...
                                <a class="font-bold hover:underline" style="margin-left: 5px" href="/{{$post->user->username}}">{{$post->user->username}}</a>
                            </div>
                            ...
                            @cannot('update', $post)
                                @livewire('follow-button', ['user_id' => $post->user->id], key($post->user->id))    
                            @endcannot
                                        ...
                                        <div class="text-gray-500 text-xs">{{$comment->created_at->format('M j o')}}
                                            @if (auth()->user()->id == $comment->user_id)
                                                ...
                                                <form action="{{route('comments.destroy',$comment->id)}}" class="inline-block" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" onclick="return confirm('Are you sure you want to delete this comment? this will delete your comment permanently.')">
                                                        <i class="fa fa-trash"></i>
                                                    </button>
                                                </form>
                                            @endif
                        ...
                        <div class="flex flex-col" id="sec3" style="margin-left: 5px">
